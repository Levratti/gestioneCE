...
$(document).ready(function () {
    ...
    // Send Mail
    document.querySelector('#mail').addEventListener('click', async () => {
        swal({
            title: "Vuoi inviarlo con Email?",
            icon: "warning",
            dangerMode: true,
            buttons: true,
        })
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    var email = "ruslan@impresalevratti.it";
                    var oggetto = JSON.parse(localStorage.CodeScan).CodeCP;
                    var messaggio = "";

                    JSON.parse(localStorage.CodeScan).CodeCE.forEach(function (code) {
                        messaggio += '%0D%0A' + code;
                    });
